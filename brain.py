...
# 시냅스 전 뉴런에 연결된 시냅스 후 뉴런의 신호 강도를 누적하는 함수
    ...
    def setup(self):
        # 모든 시냅스 후 뉴런/근육을 PostSynaptic에 초기화
        for key in constants.weights.keys():
            self.PostSynaptic[key] = [0, 0]
            for post_syn in constants.weights[key].keys():
                if post_syn not in self.PostSynaptic:
                    self.PostSynaptic[post_syn] = [0, 0]
  ...
  # ASEL ASER gustatory neurons
    ...
    def run_connectome(self): #시냅스 후에 뉴런들 검사해서 임계값 넘은 뉴런이면 발화시킴
        for PostSynaptic in self.PostSynaptic:
            if self.PostSynaptic[PostSynaptic][self.CurrentSignalIntensityIndex] > self.FireThreshold:
